...
const OportunidadesReportes: React.FC = () => {
  const [selectedPeriod, setSelectedPeriod] = useState('mes');

  // Datos de ejemplo para los gráficos
  const oportunidadesPorEstado = [
    { estado: 'Nueva', cantidad: 25 },
    { estado: 'Calificada', cantidad: 35 },
    { estado: 'Propuesta', cantidad: 20 },
    { estado: 'Negociación', cantidad: 15 },
    { estado: 'Ganada', cantidad: 30 },
    { estado: 'Perdida', cantidad: 10 },
  ];

  const oportunidadesPorValor = [
    { rango: '$0-10k', cantidad: 40 },
    { rango: '$10k-50k', cantidad: 30 },
    { rango: '$50k-100k', cantidad: 20 },
    { rango: '$100k+', cantidad: 10 },
  ];

  const ventasMensuales = [
    { mes: 'Ene', cantidad: 150000 },
    { mes: 'Feb', cantidad: 180000 },
    { mes: 'Mar', cantidad: 220000 },
    { mes: 'Abr', cantidad: 250000 },
    { mes: 'May', cantidad: 280000 },
    { mes: 'Jun', cantidad: 300000 },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-text-primary">Reportes de Oportunidades</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card-background backdrop-blur-lg p-6 rounded-2xl border border-border">
          <h3 className="text-lg font-semibold text-text-primary mb-4">Oportunidades por Estado</h3>
          <div className="space-y-4">
            {oportunidadesPorEstado.map((item, index) => (
                ...
                <div className="flex-1">
                  <div className="h-4 bg-background rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(item.cantidad / Math.max(...oportunidadesPorEstado.map(c => c.cantidad))) * 100}%` }}
                    />
                  </div>
                </div>
        ...
        <div className="bg-card-background backdrop-blur-lg p-6 rounded-2xl border border-border">
          <h3 className="text-lg font-semibold text-text-primary mb-4">Distribución por Valor</h3>
          <div className="space-y-4">
            {oportunidadesPorValor.map((item, index) => (
              <div key={index} className="flex items-center">
                ...
                <div className="flex-1">
                  <div className="h-4 bg-background rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-orange-primary to-red-primary"
                      style={{ width: `${(item.cantidad / Math.max(...oportunidadesPorValor.map(c => c.cantidad))) * 100}%` }}
                    />
                  </div>
                </div>
                <div className="w-12 text-right text-text-primary font-medium">{item.cantidad}%</div>
            ...
            {ventasMensuales.map((item, index) => (
              <div key={index} className="flex-1 flex flex-col items-center">
                <div
                  ...
                  style={{ height: `${(item.cantidad / Math.max(...ventasMensuales.map(c => c.cantidad))) * 100}%` }}
                />
          ...
          <table className="w-full">
            ...
            <tbody>
              <tr className="border-b border-border">
                ...
                <td className="py-3 text-text-primary">$1.2M</td>
              ...
              <tr className="border-b border-border">
                <td className="py-3 text-text-primary">Oportunidades Ganadas</td>
                <td className="py-3 text-text-primary">30</td>
              ...
              <tr className="border-b border-border">
                <td className="py-3 text-text-primary">Tasa de Conversión</td>
                <td className="py-3 text-text-primary">65%</td>
...
export default OportunidadesReportes; 