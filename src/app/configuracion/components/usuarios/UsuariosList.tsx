...
import { deleteEntidad } from '../../service/configuracionService';
...
const UsuariosList: React.FC = () => {
  ...
  const handleDelete = async (entidadId: number) => {
    if (!company?.id) {
      addNotification('No se encontró la compañía activa.', 'error');
      return;
    }
    try {
      const res = await deleteEntidad(entidadId, company.id);
